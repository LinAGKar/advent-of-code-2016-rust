...
fn main() {
    ...
    for i in 0..password.len() {
        let target = (2 * i + if i >= 4 { 2 } else { 1 }) % password.len();
        rots[target] = ((target - i) % password.len() + password.len()) % password.len();
    }
    ...
    for line in input.lines().rev() {
        ...
        match words.next().unwrap() {
            ...
            "move" => {
                ...
                let val = password.remove(dest).unwrap();
                password.insert(src, val);
            }
